...
@Component({
  selector: 'app-buscarcomisiones',
  templateUrl: './buscarcomisiones.component.html',
  styleUrls: ['./buscarcomisiones.component.css']
})
export class BuscarcomisionesComponent implements OnInit {
  ...
  displayedColumns = ['id', 'nombre', 'fecha_hora_inicio', 'fecha_hora_fin', 'tipos_herencia_promo', 'tipo_condicion'];
  ...
  busqueda_promociones() {
    ...
    debugger;
    this.heroService.ServicioPostGeneral("busqueda_comisiones_vend", creadoobj).subscribe((value) => {
      //  setTimeout(() => {
  ...
  goTodetalle(id: number) {
    debugger;
    this.router.navigate(['/config-comisiones/' + id]);
  }
